...
class McpGateway(models.AbstractModel):
    ...
    their own invoices, an accountant sees the books. ``sudo()`` is never used on the way in.
    """
    _name = "mcp.gateway"
    ...
    def execute_as(self, login, model, method, args=None, kwargs=None):
        if not self.env.user.has_group("base.group_system"):
            raise AccessDenied(_("mcp.gateway.execute_as is reserved for the service account"))
